# CPLG-SGD v1 fresh full-vector shadow preregistration
...
`experiments/optimizer/cplg-sgd-shadow-direction-r1-config.json`, with
...
`FAIL` kills CPLG-v1. `INCONCLUSIVE` permits a fresh identity only if the final
immutable spec already contains the exact retry branch. Infrastructure failure
permits one fresh-identify retry without changing scientific semantics. No
...
Before the direction outcome was opened, the final local audit passed 89
focused CPLG Python tests, 197 production syncer tests, two authoritative-libm
helper tests, the locked release build, Rust formatting, and Ruff check/format
on every changed Python file. The full repository suite passed 1,281 Python
...
sidecar and requires exactly one OFF result row followed by exactly one ON
result row; duplicate, extra, or reordered results are fatal.
