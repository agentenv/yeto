...
*Draft assembled 2026-07-12; 2026-07-13 results folded in. Source of record for all numbers: `docs/EXP2_23.md`–`docs/EXP2_27.md`, `docs/OPTIMIZER_SEMANTICS.md`, `docs/THEORY.md`, `docs/ANCHOR_DRIFT_CONTROL.md`, `docs/BAKEOFF_RESULTS.md`, `docs/CTTN_DESIGN.md`, `docs/CAMPAIGN_HANDOFF.md`, and `experiment-results/EXP2/rda-rho-law/summary.{md,json}`. A few sections marked **PLACEHOLDER** remain (registered fresh-seed test §4.7.1; one-step buffer-orientation screen §4.11).*

## Abstract

DiLoCo-style training applies an outer optimizer to *pseudo-gradients* — merged parameter displacements produced by $H$ inner steps on each of $M$ workers. The literature treats the sync horizon $H$, staleness, and worker count as the governing variables, and engineers against staleness while engineering *for* outer momentum. We show that a single, freely measurable quantity — the temporal autocorrelation kernel $\rho_k$ of the pseudo-gradient sequence — is the state variable through which these knobs actually act. Outer Nesterov momentum is exactly a temporal filter: its update decomposes, without approximation, into a data-dependent aligned gain $A_t = 1+\mu+\mu^2 c_t$ on the current merged delta plus a transverse residual of magnitude $\mu^2 r_t\lVert\delta_t\rVert$; on a stationary kernel the aligned gain and the RMS energy amplification are closed forms in $(\mu,\rho)$. A two-term law — aligned overstep plus buffer variance accumulation — fits a 9-cell $H\times\mu$ grid on Qwen3.5-9B LoRA training with $R^2=0.90$ where aligned-only ($R^2=0.15$) and a superseded single-term law ($R^2=0.28$) fail, retrodicts the helpful-to-harmful momentum crossover in $H$, and correctly predicted on a fresh preregistered seed that a tuned-$\eta$ memoryless baseline dissolves the fixed-$\eta$ momentum advantage at long horizons. Matched sync/async runs show the pathology needs no staleness: removing all injected delay (0–2400 ms) changes final loss by $\sim 3\times 10^{-4}$ and leaves the negative-merge rate statistically unchanged, while merge-time selection sits behind a measurement wall — the per-decision action gap lies below the noise floor of any affordable probe, and captured headroom saturates at 8 eval panels (+0.2 points from 8$\to$16). A three-arm causal control strengthens this to a mechanism claim: the short-horizon poison reproduces under a *true lockstep barrier* with version-matched deltas ($+0.100$, arm A), and current-anchor differencing is ruled out (drift $\equiv 0$, empirical to this strict-quorum scheduler), so the pathology is intrinsic to outer momentum on the short-horizon pseudo-gradient sequence rather than an artifact of staleness, non-barrier overlap, or our anchor convention. On the engineering question, an exhaustive outer-optimizer bake-off — worker-consensus, second-moment (block-RMS/Yogi), scalar curvature, spectral (Iso-C), capped-Nesterov, and a directional worker-subspace variant, extended by a second wave that broadens the search to the inner optimizer (Muon) and provably-unbiased inner control variates (SCAFFOLD-lite) — finds that *none* clears the product gate (a paired win $>0.018$ on $\ge 2$ workloads with no regression past one noise floor); each helps only in the horizon regime a dynamics diagnostic predicts, so the product conclusion is to remove momentum and ship memoryless SGD-0.28, and whether anything beats tuned SGD at short $H$ is left to a pending real-Hessian oracle (CTTN) whose $\sim$40% overhead already places it outside the gate. Because the tape is free where probes are not, we also derive a zero-evaluation-cost gain controller from the measured kernel; a v1 development screen already halves the worst-case cross-horizon regret of the best fixed policy with zero per-horizon tuning.
...
Statistical honesty (independent statistical review, 2026-07-12): adjusted $R^2 = 0.798$; the $A^2$ term survives a nested F-test against the aligned-only model ($F(1,4)\approx 29.5$, $p\approx 0.006$) and removes $\sim$88% of the aligned-only residual SSE, while $H$-intercepts alone explain at most 15%. But leverage is high ($p/n = 5/9$), there is one development seed, and no independent noise estimate — so the correct claim on the development grid alone is an excellent *descriptive* fit on the measured manifold, not yet a separately identified causal decomposition of $b$ vs $v$. The identification experiments have since run: matched-$\eta_{\mathrm{eff}}$ pairs show $\mu{=}0.9$ still loses $+0.0435$ where aligned-only predicts $\approx 0$ (Section 4.13), and the full-parameter LR-gate confirms the crossover is not an LR artifact (Section 4.12) — so the variance term $v$ now has direct support, though still on limited seeds.
...
The predeclared LR-matching gate (Section 3.2) and the LoRA-rank-2 caveat (§6) are discharged on a **full-parameter** SmolLM2-135M model with a 27-cell $H\times\mu\times\eta$ sweep (`docs/CAMPAIGN_HANDOFF.md`).

**Result (LR-gate passes on a full-parameter model).** The momentum penalty is **not rescued by the $\eta$ sweep at any $(H,\mu)$**: the best-$\eta$ arm at $\mu>0$ is always worse than the best-$\eta$ arm at $\mu=0$, by **$+0.058$ to $+0.406$** across cells. Conventional learning-rate tuning therefore does not remove the effect — this kills the biggest predeclared kill criterion ("it's just LR tuning") — and the helpful-to-harmful crossover is a real dynamical phenomenon, not an LR artifact, on a full-parameter (not LoRA-rank-2) model. Two corroborating facts on the same rank-2 concern: the crossover replicates in sign on **three seeds** (223/251/283, $\mu{=}0$ wins short $H$, $\mu{=}0.5$ wins long $H$), and moving to LoRA rank 16 makes the poison **6–7$\times$ larger** ($+0.528$/$+0.435$ at H16/H64 vs rank-2 $+0.086$/$+0.062$) with higher $\rho$ (0.356 vs 0.25) — so the rank-2 "artifact" objection is refuted from the loss side (full-parameter and rank-16) as well as the dynamics-geometry side. TODO: the per-cell 27-cell table and the individual fairness-control columns are not reproduced in the committed handoff; only the aggregate penalty range ($+0.058$ to $+0.406$) is on record.

### 4.13 Matched-$\eta_{\mathrm{eff}}$ pairs and the mediation control (identifying $v$ separately from $b$)

**Result (matched effective-LR).** At *matched* corrected effective-LR — $\eta$ tuned so the aligned step $\eta_{\mathrm{eff}}(\mu,\rho)$ is equalized across $\mu$ — $\mu{=}0.9$ still **loses $+0.0435$**. The aligned-only model predicts $\approx 0$ here by construction, so aligned-only is refuted and the variance/transverse term ($v\,\Delta\log A^2_{\mathrm{RMS}}$; THEORY.md $V(\mu,\rho)$) is real and separately identified: momentum's damage is not fully absorbable into an effective learning rate.

**Result (mediation, exp2-39 — closing the mechanism from the norm side).** A parallel norm-match control closes the mechanism causally (`docs/CAMPAIGN_HANDOFF.md`). At a *fixed applied merged norm* $R = 10.4436$ (every norm-matched arm applies this exact merged-step norm), the spread across inner-LR is

$$1.531921 - 1.368771 = \mathbf{+0.163} \;\gg\; 0.009,$$

so matching the applied norm does **not** remove the damage — it is pseudo-gradient *correlation geometry*, not norm amplification (the norm-ref control is inert at the reference inner-LR: normmatch-base 1.418508 $\approx$ base-ref 1.418466). And the momentum dose-response at inner-LR $2\times10^{-3}$ is monotone,

$$\underbrace{1.382111}_{\mu=0} \;<\; \underbrace{1.430336}_{\mu=0.5} \;<\; \underbrace{1.625482}_{\mu=0.9},$$

a $+0.243$ rise from $\mu{=}0$ to $\mu{=}0.9$ at high inner-LR — the two-channel (single-$Z$-falsified) causal closure the two-term law needed.

**Honesty caveat.** The inner-LR poison axis is *not* mediated by $\rho$: loss swings 1.362$\to$1.625 across inner-LR (0.0005/0.001/0.002) while $\rho$ stays flat (0.234 vs 0.232). So $\rho$ prices the momentum-vs-horizon filter interaction but is **not by itself the sufficient state variable** — the additional mediator on the inner-LR axis is likely delta-norm / curvature, consistent with the directional CTTN framing (Section 4.18) and flagged as a limitation (§6).

### 4.14 Dynamic-H / elastic synchronization
...
**Result.** Fixed outer Nesterov $\mu{=}0.9$ **destabilizes at each horizon switch**: the post-switch applied-direction cosine collapses to **0.374** (vs SGD's **1.00** and capped-Nesterov's **0.873**) — the stale buffer, tuned to the old horizon's kernel, points nearly orthogonal to the new pseudo-gradient. Tuned SGD wins the dynamic-$H$ workload outright; the best controller trails tuned SGD by only **$+0.011$** and leads fixed Nesterov by $+0.056$, absorbing switches with no per-horizon tuning. Momentum is thus not merely worse in level at short $H$ — it is *transiently unstable* precisely when the horizon (hence $\rho$) changes under it, while a gain-/curvature-capped controller degrades gracefully.
...
- **Controller status.** v1 results are a same-seed development screen with a known miscalibration; no frozen-controller fresh-seed evidence exists yet. The frozen capped-Nesterov v2.1 is only a safety device (Section 4.10): H-invariant but never a win. The wall claim is about affordable probes for closely spaced merge actions after early training, not about selection in general (early-training selection demonstrably works, 56–84% headroom).
- **$\rho$ is not by itself the sufficient state variable.** The inner-LR poison axis is not mediated by $\rho$: loss swings 1.362$\to$1.625 across inner-LR while $\rho$ stays flat (0.234 vs 0.232, Section 4.13). $\rho$ prices the momentum-vs-horizon filter interaction, but a second geometry variable (likely delta-norm / curvature, per the exp2-39 mediation and the CTTN framing) governs the inner-LR axis; the kernel is a state variable *for the filter*, not a universal one for all of the dynamics.
- **Product scope and the pending oracle.** "Ship SGD-0.28" rests on the bake-off (Section 4.17), in which no candidate clears the product gate; each wins only in one horizon regime, and a few arms are incomplete (block-Yogi H256 to preemption; the wsub sweep beyond its H16 corner) though the verdict is robust to them. Whether *any* method beats tuned SGD at short $H$ is not settled: the CTTN real-Hessian oracle (Section 4.18) is in progress and, at $\sim$40% overhead, is a research lower-bound instrument, not a shippable optimizer.
