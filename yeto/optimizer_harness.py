...
def _post_delete_campaign_accelerator_proof(
    spec: ExperimentSpec,
    runner: CommandRunner,
    state: Mapping[str, Any],
) -> dict[str, Any]:
    """Prove this campaign owns no remaining accelerator; retain unrelated count."""
    result = runner.run(
        [
            "gcloud",
            "compute",
    ...
    if runner.dry_run:
        ...
        try:
            ...
            raise HarnessError(
                f"gcloud returned invalid post-delete inventory JSON: {exc}"
            ) from exc
        if not isinstance(instances, list):
            raise HarnessError("post-delete instance inventory must be a JSON array")
    expected_labels = _expected_ownership_labels(spec, state)
    total = 0
    campaign_owned = 0
    for index, instance in enumerate(instances):
        if not isinstance(instance, dict):
            raise HarnessError(f"post-delete inventory row {index} is not an object")
        if instance.get("status") == "TERMINATED":
            continue
        row_accelerators = 0
        accelerators = instance.get("guestAccelerators", [])
        if not isinstance(accelerators, list):
            raise HarnessError("post-delete guestAccelerators must be an array")
        for accelerator in accelerators:
            if not isinstance(accelerator, dict):
                raise HarnessError("post-delete accelerator row is malformed")
            try:
                count = int(accelerator.get("acceleratorCount"))
            except (TypeError, ValueError) as exc:
                raise HarnessError("post-delete acceleratorCount is invalid") from exc
            if count < 0:
                raise HarnessError("post-delete acceleratorCount may not be negative")
            row_accelerators += count
        total += row_accelerators
        labels = instance.get("labels")
        if isinstance(labels, dict) and all(
            labels.get(key) == value for key, value in expected_labels.items()
        ):
            campaign_owned += row_accelerators
    if campaign_owned != 0:
        raise HarnessError(
            "post-delete campaign-owned accelerator proof is nonzero: "
            f"owned={campaign_owned}"
        )
    return {
        "project": spec.project,
        "queried_at_utc": _utc_now(),
        "campaign_owned_accelerators": campaign_owned,
        "total_active_accelerators": total,
        "unrelated_active_accelerators": total - campaign_owned,
...
def launch(
    ...
    *,
    ...
    state["provisioning_started_at_utc"] = provisioning_started_at
    state["provisioning_completed_at_utc"] = _utc_now()
...
def _provider_evidence(
    ...
    scheduling = _object(description.get("scheduling", {}), "instance scheduling")
    machine_type = str(description.get("machineType", spec.cloud["machine_type"]))
    evidence = {
        "schema": "yeto_gcp_spot_evidence_v1",
        ...
        "instance_type": machine_type.rsplit("/", 1)[-1],
        "instance_self_link": state["instance_self_link"],
        "boot_disk_self_link": state["boot_disk_self_link"],
        "boot_disk_name": _recorded_boot_disk_name(state),
        "boot_disk_id": state.get("boot_disk_id"),
        "source_image_id": state.get("source_image_id"),
        "provisioning_started_at": state.get("provisioning_started_at_utc"),
        "provisioning_completed_at": state.get("provisioning_completed_at_utc"),
        "provisioning_model": scheduling.get("provisioningModel"),
        "instance_termination_action": scheduling.get("instanceTerminationAction"),
        "automatic_restart": scheduling.get("automaticRestart"),
        "on_host_maintenance": scheduling.get("onHostMaintenance"),
        "labels": dict(description.get("labels", state.get("labels", {}))),
    }
    if evidence["provisioning_model"] != "SPOT":
        raise HarnessError("provider evidence does not prove Spot provisioning")
    if evidence["instance_termination_action"] != "DELETE":
        raise HarnessError("provider evidence does not prove DELETE termination")
    return evidence
...
def start_script(
    ...
    if provider_evidence is not None:
        encoded_evidence = base64.b64encode(
            (json.dumps(dict(provider_evidence), indent=2, sort_keys=True) + "\n").encode()
        ).decode()
        evidence_setup = (
...
{evidence_setup}{input_provenance_copy}
{{
{input_manifest_verification}
...
def start(
    ...
    evidence = None if runner.dry_run else _provider_evidence(spec, state, description)
    result = runner.run(ssh_command(spec, start_script(spec, evidence)))
    if runner.dry_run:
        return {"dry_run": True}
    try:
    ...
    if evidence is not None:
        ...
        state["provider_evidence_sha256"] = hashlib.sha256(
            evidence_payload.encode()
        ).hexdigest()
        state["provider_evidence_uri"] = (
            spec.artifact_uri + "/provider-evidence.json"
        )
...
def verify_artifact_roundtrip(
    ...
    """Prove critical sealed files are byte-identical in GCS before deletion."""
    run_root = spec.remote_run_dir.rstrip("/") + "/"
    paths = [
        f"{spec.remote_run_dir}/spec.json",
        f"{spec.remote_run_dir}/command.sh",
        f"{spec.remote_run_dir}/provider-evidence.json",
        f"{spec.remote_run_dir}/runner.exit",
        f"{spec.remote_run_dir}/final-manifest.sha256",
        *spec.execution["completion_paths"],
    ...
    for path in dict.fromkeys(paths):
        if not path.startswith(run_root):
            raise HarnessError(
                f"cannot round-trip artifact outside remote run directory: {path}"
            )
        relative = path[len(run_root) :]
        uri = f"{spec.artifact_uri}/{relative}"
        checks.append(
            f"test -f {shlex.quote(path)} && "
            f"gcloud storage cat {shlex.quote(uri)} | cmp - {shlex.quote(path)}"
        )
...
def seal_phase_map_artifact_objects(
    spec: ExperimentSpec,
    runner: CommandRunner,
    state: Mapping[str, Any],
) -> dict[str, Any] | None:
    """Bind P0 teardown to immutable generations of its two root seals."""
    declared = [
        *spec.execution["completion_paths"],
        *spec.execution["checksum_manifests"],
    ...
    role_by_name = {
        "phase-map-manifest.json": "phase_map_manifest",
        "acquisition.sha256": "acquisition_checksum",
    }
    selected: dict[str, str] = {}
    for remote in declared:
        role = role_by_name.get(PurePosixPath(remote).name)
        if role is not None:
            if role in selected and selected[role] != remote:
                raise HarnessError(f"multiple declared artifacts claim role {role}")
            selected[role] = remote
    phase_map_run = any("run_phase_map.py" in part for part in spec.command)
    if not selected and not phase_map_run:
        return None
    if set(selected) != set(role_by_name.values()):
        raise HarnessError(
            "phase-map teardown requires declared phase-map-manifest.json and "
            "acquisition.sha256 completion artifacts"
        )
    objects = []
    for role, remote in sorted(selected.items()):
        try:
            relative = PurePosixPath(remote).relative_to(
                PurePosixPath(spec.remote_run_dir)
            )
        except ValueError as exc:
            raise HarnessError(
                f"phase-map seal artifact is outside remote run directory: {remote}"
            ) from exc
        uri = f"{spec.artifact_uri}/{relative.as_posix()}"
        described = runner.run(
            ["gcloud", "storage", "objects", "describe", uri, "--format=json"]
        )
        content = runner.run(["gcloud", "storage", "cat", uri])
        if runner.dry_run:
            metadata = {
                "generation": "DRY_RUN",
                "metageneration": "DRY_RUN",
                "size": 1,
            }
        else:
            metadata = _json_object_from_result(
                described, f"GCS metadata for {role}"
            )
        ...
        try:
            ...
            raise HarnessError(f"GCS {role} metadata has invalid size") from exc
        if not runner.dry_run and (
            not re.fullmatch(r"[0-9]+", generation)
            or not re.fullmatch(r"[0-9]+", metageneration)
            or size <= 0
        ):
            raise HarnessError(f"GCS {role} metadata lacks immutable generation fields")
        raw = (content.stdout or "").encode("utf-8")
        if not runner.dry_run and len(raw) != size:
            raise HarnessError(f"GCS {role} content length differs from metadata")
        objects.append(
            {
                "role": role,
                "uri": uri,
...
def analyze(
    ...
    if state.get("status") != "DELETED":
        raise HarnessError("CPU analysis is allowed only after exact VM deletion")
    if not state.get("verified_instance_absent") or not state.get(
        "verified_boot_disk_absent"
    ):
        raise HarnessError(
            "CPU analysis requires verified absence of both VM and boot disk"
        )
    if not spec.analysis:
        raise HarnessError("spec defines no analysis hooks")
    local_root = Path(
        os.environ.get(
            "YETO_OPTIMIZER_ARTIFACT_DIR",
            f".optimizer-harness/artifacts/{spec.run_id}",
        )
    ).resolve()
    if local_root.exists() and any(local_root.iterdir()):
        raise HarnessError(f"local analysis artifact directory is not empty: {local_root}")
    ...
    for hook in spec.analysis:
        runner.run(hook["command"], capture=False)
...
def delete(
    ...
    *,
    ...
    if runner.dry_run:
        disk_id = str(state.get("boot_disk_id", "DRY_RUN_DISK_ID"))
    else:
        _verify_abandonment_attachment(spec, state, description)
        disk = _json_object_from_result(
            runner.run(disk_describe_command(spec, disk_name)),
            "boot disk description",
        )
        disk_id = _numeric_provider_id(disk.get("id"), "boot disk id")
        if disk.get("selfLink") != state["boot_disk_self_link"]:
            raise HarnessError("REFUSING: boot disk self-link changed before delete")
    if description.get("status") == "RUNNING":
        ...
        artifact_seal = seal_phase_map_artifact_objects(spec, runner, state)
        if artifact_seal is not None:
            state["artifact_object_seal"] = artifact_seal
            save_state(spec, state, state_dir)
    requested_at = _utc_now()
    state["deletion_requested_at_utc"] = requested_at
    save_state(spec, state, state_dir)
    runner.run(delete_command(spec))
    if runner.dry_run:
        return None
    _verify_absent(runner, describe_command(spec), "experiment instance")
    _verify_absent(
        runner,
        disk_describe_command(spec, disk_name),
        "experiment boot disk",
    )
    absence_verified_at = _utc_now()
    accelerator_proof = None
    if state.get("artifact_object_seal") is not None:
        accelerator_proof = _post_delete_campaign_accelerator_proof(
            spec, runner, state
        )
    state["status"] = "DELETED"
    state["deleted_at_utc"] = _utc_now()
    state["deleted_instance_id"] = recorded
    state["deleted_instance_name"] = spec.instance_name
    state["deleted_boot_disk_id"] = disk_id
    state["deleted_boot_disk_name"] = disk_name
    state["verified_instance_absent"] = True
    state["verified_boot_disk_absent"] = True
    state["provider_not_found_verification"] = {
        "instance": {
            "name": spec.instance_name,
            "provider_id": recorded,
            "result": "NOT_FOUND",
            "verified_at_utc": absence_verified_at,
        },
        "boot_disk": {
            "name": disk_name,
            "provider_id": disk_id,
    ...
    if accelerator_proof is not None:
        state["post_delete_accelerator_proof"] = accelerator_proof
    save_state(spec, state, state_dir)
    return state
