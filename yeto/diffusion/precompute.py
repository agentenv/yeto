    ...
    with manifest.open("w", encoding="utf-8") as f:
        for rows in loader:
            ...
            if args.cache_latents:
                ...
                latents = learner.encode_latents(pipe, rows, args, device, dtype).detach().cpu()
                args.cache_latents = old
            if args.cache_text_embeds:
                old = args.cache_text_embeds
                args.cache_text_embeds = False
                prompt_embeds, pooled = learner.encode_prompt_embeds(pipe, rows, args, device, dtype)
                prompt_embeds = prompt_embeds.detach().cpu() if prompt_embeds is not None else None
                pooled = pooled.detach().cpu() if pooled is not None else None
            ...
            for b, row in enumerate(rows):
                ...
                if latents is not None:
                    ...
                    item["height"] = int(latents[b].shape[-2])
                    item["width"] = int(latents[b].shape[-1])
                    if latents[b].ndim == 4:
                        item["frames"] = int(latents[b].shape[-3])
