...
DIFFUSION_CAPABILITIES: dict[str, DiffusionCapability] = {
    ...
    "flux-schnell": _cap(
        ...
        gaps=("training target distribution differs from Flux dev; validate before recommending",),
        evidence=("finetrainers Flux notes",),
    ),
    "flux2-dev": _cap(
        family="Flux2",
    ...
    "hidream-i1-dev": _cap(
        ...
        status="generic-gap",
        gaps=("multi-encoder prompt tuple mapping and real component loading need validation",),
    ...
    "ideogram4": _cap(
        ...
        gaps=("broader production training still needs validation",),
        evidence=("Verda A100 ideogram4 one-step train/backward/save", "diffusers examples/dreambooth/train_dreambooth_lora_ideogram4.py"),
    ),
    "qwen-image": _cap(
        family="Qwen Image",
    ...
    "sd35": _cap(
        ...
        status="needs-real-validation",
        gaps=("real SD3.5 LoRA GPU validation still pending",),
        evidence=("diffusers examples/dreambooth/train_dreambooth_lora_sd3.py",),
    ),
    "ltx-video": _cap(
        family="LTX Video",
    ...
    "ltx2-video": _cap(
        ...
        conditioning=("prompt_embeds", "audio_hidden_states", "audio_encoder_hidden_states"),
        latent_layout="packed 3D video latent tokens with audio conditioning",
        scheduler=_FLOW_MATCH,
        forward_kwargs=("hidden_states", "timestep", "encoder_hidden_states", "audio_hidden_states", "audio_encoder_hidden_states"),
        output_alignment="packed-token target",
        status="adapter-required",
        adapter_boundary="Yeto can forward externally prepared audio tensors, but raw audio feature extraction is model-specific.",
        evidence=("Diffusers LTX2 contract inspection",),
    ),
    "hunyuan-video": _cap(
    ...
    "motif-video": _cap(
        ...
        latent_layout="3D video latents; in_channels hints possible conditioning concat",
        scheduler=_FLOW_MATCH,
        forward_kwargs=("hidden_states", "timestep", "encoder_hidden_states"),
        output_alignment="unknown",
        status="needs-real-validation",
        gaps=("in_channels=33-style latent contract must be tested before generic support claim",),
        evidence=("Hub config/source inspection",),
    ),
    "wan21-t2v-1.3b": _cap(
        family="Wan",
        ...
        output_alignment="direct latent output",
        status="generic-covered",
        gaps=("broader production-length validation still pending",),
        evidence=("AWS g6e Wan2.1 1.3B raw-video train/backward/save", "finetrainers WanModelSpecification"),
    ),
    "wan21-t2v-14b": _cap(
        family="Wan",
        pipeline="WanPipeline",
    ...
    "cogvideox-2b": _cap(
        ...
        modalities=("video", "text"),
        conditioning=("prompt_embeds",),
        latent_layout="video latents ordered as B,F,C,H,W",
        scheduler="DDIM/DDPM add_noise plus velocity conversion",
        forward_kwargs=("hidden_states", "timestep", "encoder_hidden_states", "image_rotary_emb", "ofs"),
        output_alignment="scheduler.get_velocity output vs latent target",
        status="generic-gap",
        gaps=("image_rotary_emb/ofs generation and velocity target path need generic support",),
        evidence=("diffusers examples/cogvideo/train_cogvideox_lora.py", "finetrainers CogVideoXModelSpecification"),
    ),
    "cogvideox-5b": _cap(
        family="CogVideoX",
        pipeline="CogVideoXPipeline",
    ...
    "cogview4-6b": _cap(
        ...
        modalities=("image", "text"),
        conditioning=("prompt_embeds",),
        latent_layout="packed 2D image latent tokens",
        scheduler="flow matching with resolution-dependent shifted sigmas",
        forward_kwargs=("hidden_states", "timestep", "encoder_hidden_states"),
        output_alignment="direct latent output",
        status="generic-gap",
        gaps=("resolution-dependent sigma shift needs generic scheduler/config expression",),
        evidence=("finetrainers CogView4ModelSpecification",),
    ),
    "nava": _cap(
        family="NAVA",
