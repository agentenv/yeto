...
class _TrainTelemetry:
    """The complete scalar surface allowed to leave the Miles train rank.

    Miles' authoritative ``log_train_step`` dictionary is much wider than the
    loss curve we publish.  Converting it to this closed, typed capsule on the
    one metrics-owner rank makes it impossible for prompt, response, tool, or
    arbitrary future metric fields to hitch a ride through Ray to W&B.
    """
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("training telemetry step must be a non-negative integer")
        values = (
            self.loss,
            self.pg_loss,
            self.grad_norm,
        ...
        if not any(value is not None for value in values):
            raise ValueError("training telemetry must contain an allowlisted scalar")
        if any(value is not None and not math.isfinite(value) for value in values):
            raise ValueError("training telemetry scalars must be finite")
        if self.pass_rate is not None and not 0.0 <= self.pass_rate <= 1.0:
            raise ValueError("training pass rate must be in [0, 1]")
...
def _apply_progress_due(completed: int, total: int) -> bool:
    ...
    stride = max(
        1,
        (total + _MAX_APPLY_PROGRESS_MARKERS - 1) // _MAX_APPLY_PROGRESS_MARKERS,
    )
...
class _TrainableStateFragment:
    ...
    telemetry: _TrainTelemetry | None = None
...
def _logical_mapping_hf_names(task: Any) -> tuple[str, ...]:
    return tuple(
        training_to_logical_expert_name(name) for name in _mapping_hf_names(task)
    )
...
def filter_selected_expert_tasks(tasks, *, expert_count: int) -> list[Any]:
    ...
    for task in tasks:
        ...
        if names and all(
            selected_expert_hf_name(name, expert_count=expert_count) for name in names
        ):
...
def _validate_hybrid_names(tensors: Mapping[str, Any], expert_count: int) -> None:
    ...
    if outside:
        raise ValueError(
            f"expert tensor is outside the selected clone policy: {outside[0]!r}"
        )
    missing = sorted(expected_experts - actual_experts)
    if missing:
        raise ValueError(f"hybrid policy is missing expert tensor {missing[0]!r}")
    lora = [
        name for name in tensors if name.endswith((".lora_A.weight", ".lora_B.weight"))
    ]
    if not lora:
...
_TRAIN_TELEMETRY_ALIASES = {
    ...
    "pass_rate": ("train/pass_rate", "train/pass@1"),
}


def _finite_train_metric(
    metrics: Mapping[str, Any], names: tuple[str, ...]
) -> float | None:
    for name in names:
...
def _train_lr(metrics: Mapping[str, Any]) -> float | None:
    direct = _finite_train_metric(
        metrics,
        ("train/lr-pg_0", "train/lr", "train/learning_rate", "train/policy_lr"),
    )
    ...
    groups = sorted(
        (
            ...
            for name in metrics
            if (match := re.fullmatch(r"train/lr-pg_([0-9]+)", str(name))) is not None
        ),
        key=lambda item: item[0],
    )
    return _finite_train_metric(metrics, tuple(name for _group, name in groups))


def _extract_train_telemetry(metrics: object) -> _TrainTelemetry | None:
    """Narrow Miles' rank-owner dictionary without retaining raw input."""
    ...
    if (
        ...
        return None
    values = {
        field: _finite_train_metric(metrics, aliases)
        for field, aliases in _TRAIN_TELEMETRY_ALIASES.items()
    }
    values["lr"] = _train_lr(metrics)
    if values["pass_rate"] is not None and not 0.0 <= values["pass_rate"] <= 1.0:
        values["pass_rate"] = None
    if not any(value is not None for value in values.values()):
        return None
    try:
        return _TrainTelemetry(step=step, **values)
    except (TypeError, ValueError):
        # Telemetry is observability, never a reason to fail a training step.
        return None


def _capture_external_train_telemetry(
    args,
    metrics: object,
    *,
    is_metrics_source: bool,
) -> None:
    """Replace Miles' narrow capture at its existing one-owner call site.

    Pinned Miles calls ``_capture_external_train_metrics`` only from the first
    replica Megatron main rank, immediately after constructing the authoritative
    ``log_train_step`` dictionary.  Retain only a closed typed capsule so
    arbitrary future fields cannot cross the Ray boundary.
    """

    if (
        not is_metrics_source
        or getattr(args, "external_policy_sync_path", None) is None
    ):
        return
    try:
        telemetry = _extract_train_telemetry(metrics)
    except Exception:  # noqa: BLE001 - observability never breaks training
        telemetry = None
    # Miles clears this field to ``{}`` at the start of every train call.  A
    # successful producer point replaces it with the bounded typed capsule;
    # no parallel raw dictionary is retained.
    args._external_train_metrics = telemetry if telemetry is not None else {}


def _state_telemetry(state) -> _TrainTelemetry | None:
    telemetry = getattr(state, "telemetry", None)
    if telemetry is not None and not isinstance(telemetry, _TrainTelemetry):
        raise RuntimeError("hybrid state contains invalid training telemetry")
    return telemetry
...
def _merge_export_fragments(module: ModuleType, fragments):
    ...
    if any(
        ...
        or _state_telemetry(fragment) is not None
    ...
    for fragment in sorted(fragments, key=lambda item: item.source_rank):
        ...
        if overlap:
            raise RuntimeError(
                f"hybrid state fragments contain duplicate tensor {min(overlap)!r}"
            )
    ...
    merged = make_hybrid_trainable_state(
        module,
        root.policy_version,
        tensors,
    ...
    return _OwnedTrainableState(
        ...
        layout_hash=merged.layout_hash,
        tensors=merged.tensors,
        **_state_metrics(merged),
        telemetry=_state_telemetry(metrics_source),
    )
...
def _chunk_export_fragment_for_ray(
    ...
    *,
    ...
    for group in groups:
        local = {
            name: fragment.tensors[name] for name in group if name in fragment.tensors
        }
        if not local:
            continue
    ...
    return _ChunkedExportFragment(
        ...
        **_state_metrics(fragment),
        telemetry=_state_telemetry(fragment),
    )
...
def _validate_chunked_export_metadata(fragments):
    ...
    for fragment in fragments:
        ...
        if (
            not names
            or any(
                not names_in_chunk or names_in_chunk != tuple(sorted(names_in_chunk))
                for names_in_chunk in fragment.chunk_tensor_names
            )
...
def _take_chunked_export_tensors(
    ...
    *,
    ...
    if not requested <= export._remaining_names:
        raise RuntimeError(
            "chunked export tensor group was already consumed or unknown"
        )
        ...
        for chunk_index, (chunk_names, expected_bytes, reference) in enumerate(
            ...
            try:
                ...
                for name, tensor in chunk.items():
                    ...
                    if not isinstance(tensor, torch.Tensor):
                        raise TypeError(
                            f"hybrid policy tensor {name!r} is not a tensor"
                        )
                    value = tensor.detach() if tensor.requires_grad else tensor
                    if (
                        value.device.type != "cpu"
...
def _plan_state_chunks(state, *, max_chunk_bytes: int):
    ...
    for name in names:
        ...
        if tensor_bytes > max_chunk_bytes:
            raise ValueError(f"hybrid policy tensor {name!r} exceeds the Ray chunk cap")
...
def _state_from_chunk_manifest(
    ...
    *,
    ...
    for index, (chunk, expected_names, expected_bytes) in enumerate(
        ...
        actual_bytes = sum(
            _tensor_bytes(name, tensor) for name, tensor in chunk.items()
        )
...
def install_on_model(module: ModuleType) -> None:
    ...
    def capture_external_train_metrics(args, metrics):
        # This symbol is the exact call target in Miles' head-gated train loop;
        # repeat the ownership check here so a future call site cannot duplicate
        # or retain telemetry on another rank.
        owner = getattr(module, "is_first_replica_megatron_main_rank", None)
        _capture_external_train_telemetry(
            args,
            metrics,
            is_metrics_source=bool(owner is not None and owner()),
        )

    module.get_megatron_optimizer = get_megatron_optimizer
    module._capture_external_train_metrics = capture_external_train_metrics
    module._yeto_expert_full_installed = True
...
def _expert_views(actor) -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        names = tuple(_canonical_name(name) for name in _logical_mapping_hf_names(task))
        projections = {}
        for name in names:
            match = _EXPERT_WEIGHT.fullmatch(name)
    ...
    with torch.no_grad():
        for spec in specs:
            ...
            if side.param_weight is not None or dist.get_rank() == 0:
                local[spec.name] = (
                    exported[spec.name].detach().to(device="cpu").contiguous()
                )
    ...
    *,
    ...
    for spec in _expert_specs(actor):
        ...
        if (
            ...
            raise RuntimeError(f"DP replicas disagree on expert tensor {spec.name!r}")
...
def export_hybrid_trainable_state(module: ModuleType, actor, *, policy_version: int):
    ...
    if (
        is_metrics_source
        and getattr(actor.args, "external_policy_sync_path", None) is not None
    ):
        metrics = getattr(actor.args, "_external_train_metrics", {})
    telemetry = (
        metrics
        if isinstance(metrics, _TrainTelemetry)
        else _extract_train_telemetry(metrics)
    )
    legacy_metrics = metrics if isinstance(metrics, Mapping) else {}
    return _TrainableStateFragment(
        ...
        train_rollout_kl=(
            ...
            else legacy_metrics.get("train/train_rollout_kl")
        ),
        ess_ratio=(
            telemetry.ess
            if telemetry is not None
            else legacy_metrics.get("train/ess_ratio")
        ),
        pg_clipfrac=(
            telemetry.clipfrac
            if telemetry is not None
            else legacy_metrics.get("train/pg_clipfrac")
        ),
        train_seconds=(
        ...
        telemetry=telemetry if is_metrics_source else None,
    )
...
def _validate_chunk_manifest(module: ModuleType, actor, manifest) -> None:
    ...
    flattened = tuple(name for names in manifest.chunk_tensor_names for name in names)
    ...
    if dist.get_rank() == 0:
        try:
            tensors = _validated_chunk_tensors(
                ...
                expected_shapes={
                    name: tuple(specs_by_name[name].shape) for name in expected_names
                },
...
def install_on_actor_group(module: ModuleType) -> None:
    ...
    module.RayTrainGroup.export_trainable_state_chunks = export_trainable_state_chunks
...
def _weight_iterator_task_bridge(iterator):
    ...
    if not source:
        raise RuntimeError(
            "expert-full weight iterator has no HuggingFace checkpoint for Bridge tasks"
        )
    from megatron.bridge import AutoBridge
