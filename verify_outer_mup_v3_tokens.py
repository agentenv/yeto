#!/usr/bin/env python3
"""Verify v3 shard integrity and exact SmolLM2 packed-token capacity."""
...
EXPECTED_SEEDS = tuple(range(301, 332)) + tuple(range(401, 410))
...
def main() -> int:
    ...
    parser.add_argument(
        "--root", type=Path, default=Path("/root/yeto-data/outer-mup-v3")
    )
    parser.add_argument(
    ...
    for seed in EXPECTED_SEEDS:
        ...
        manifest_path = seed_root / "input-manifest.json"
        manifest = json.loads(manifest_path.read_text())
        provenance_path = seed_root / "split_provenance.json"
    ...
    result = {
        "schema": "yeto_outer_mup_v3_token_verification_v1",
        "status": "PASS",
        "pipeline": {
