...
def _endpoint(store: CaptureObjectStore):
    ...
    return publish_learner_endpoint(
        ...
        future_groups=FutureGroupRefs(
            "incomplete", {}, "future groups unavailable in policy fixture"
        ),
    )


def _boundary(store: CaptureObjectStore, *, weight_f64_bits=None):
...
def _action_arguments(store: CaptureObjectStore, *, capabilities=CAPABILITIES):
    ...
    return {
        ...
        "decision_sha256": hashlib.sha256(b"decision record").hexdigest(),
...
def test_nonstock_action_is_canonical_and_contains_no_outcome_fields(tmp_path):
    ...
    assert loaded.required_capabilities == (
        "same_fragment_history",
        "global_boundary_state",
    )
...
def test_missing_policy_capability_rejects_publication_and_resealed_action(tmp_path):
    store = CaptureObjectStore(tmp_path / "cas")
    arguments = _action_arguments(store, capabilities=("same_fragment_history",))
    ...
    arguments["required_capabilities"] = ["same_fragment_history"]
...
def test_resealed_action_schema_role_and_cross_wire_mutations_fail_closed(
    ...
    if case == "extra-outcome-field":
        ...
        metadata["required_capabilities"].reverse()
