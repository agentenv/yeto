...
SPEC_PATH = (
    ROOT / "experiments" / "optimizer" / "exp2-cplg-shadow-direction-r1.json"
)
CONFIG_PATH = (
    ROOT
    / "experiments"
    / "optimizer"
    / "cplg-sgd-shadow-direction-r1-config.json"
)
DOSSIER_PATH = (
    ROOT / "experiments" / "optimizer" / "cplg-sgd-shadow-prereg.md"
)
COMMIT = "a4122c5f6caffb03636e114609e2feb136a7f5ae"
CONFIG_SHA256 = "6636a9a537cfac495c2f4c22f641b2a325f4e79e78da3d876ff70595b49f79d8"
CAPTURE_SESSION = "a758d49d-203d-4e80-a816-0ffe38f60588"
RUN = "/home/shou/runs/exp2-cplg-shadow-direction-r1"
REPO = "/home/shou/experiments/exp2-cplg-shadow-direction-r1/repo"
...
def test_cloud_spec_is_pinned_to_frozen_implementation_and_scientific_files() -> None:
    ...
    assert spec.run_id == "exp2-cplg-shadow-direction-r1"
    ...
    assert bindings["dossier_sha256"] == hashlib.sha256(
    ...
    assert spec.artifact_uri.endswith("/exp2-cplg-shadow-direction-r1")
...
def test_completion_contract_covers_every_vector_and_primary_receipt() -> None:
    ...
    for commit, path in enumerate(vectors, 1):
        assert path.endswith(
            f"commit-{commit:08}-fragment-{(commit - 1) % 4:04}.f32le"
        )
    tape_manifest = (
        f"{RUN}/work/cplg_shadow_on/stock_vectors/stock_tape.manifest.json"
    )
