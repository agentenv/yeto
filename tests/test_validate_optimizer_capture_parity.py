...
SPEC = importlib.util.spec_from_file_location("validate_optimizer_capture_parity", SCRIPT)
...
def _probe_row(
    *, state_name: str, candidate_name: str, update_name: str
) -> dict[str, object]:
    return {
...
def _make_probe(root: Path, *, prefix: str, candidate: bytes, update: bytes) -> None:
    state_name = f"{prefix}-state.ckpt"
    candidate_name = f"{prefix}-candidate.f32"
    update_name = f"{prefix}-update.f32"
    for directory in ("states", "candidates", "applied_updates"):
        (root / directory).mkdir(parents=True)
    (root / "states" / state_name).write_bytes(
        _checkpoint_bytes(global_step=0, version=0)
    )
...
def _make_pair(tmp_path: Path, *, off_wall: float = 100.0, on_wall: float = 101.8) -> Pair:
    ...
    _make_probe(on_arm / "syncer_probe", prefix="on", candidate=candidate, update=update)
...
def test_matched_pair_passes_with_only_known_path_metadata_canonicalized(tmp_path):
    ...
    assert pair.output.with_name(pair.output.name + ".sha256").read_text(
        encoding="ascii"
...
def test_transcript_must_cover_every_indexed_responder(tmp_path):
    ...
    for arm in (pair.off_arm, pair.on_arm):
        ...
        rows = [json.loads(probe.joinpath("index.jsonl").read_text(encoding="utf-8"))]
        second = dict(rows[0])
        second["learner_id"] = 1
...
def test_nominal_under_two_percent_fails_when_rounding_bound_is_over(tmp_path):
    pair = _make_pair(tmp_path, off_wall=100.0, on_wall=101.9)

    result = _run(pair)

    check = _check(result, "eval_and_wall_overhead")
    assert result["status"] == "FAIL"
    assert "rounding-worst" in check["error"]
