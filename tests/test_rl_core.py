...
def test_round_audit_is_opt_in_identity_bound_and_idempotent(tmp_path):
    runtime = type("Runtime", (), {"initialize": lambda self: lora_tensors()})()
    bridge = StrictRlBridge(
        runtime,
        BridgeConfig(
    ...
    delta = torch.arange(14, dtype=torch.float32)
    cached = bridge.cache.save(
        base_identity=bridge.initial.identity,
        target_step=1,
        delta=delta,
        stats={},
    )
    bridge._record_round_audit(base=bridge.initial, cached=cached)
    bridge._record_round_audit(base=bridge.initial, cached=cached)

    metadata = json.loads((tmp_path / "audit/round-00000001.json").read_text())
    assert metadata["base_policy_hash"] == bridge.initial.policy_hash
    assert metadata["delta_sha256"] == cached.delta_sha256
    assert (tmp_path / "audit/round-00000001.base.f32").stat().st_size == 14 * 4
    assert (tmp_path / "audit/round-00000001.delta.f32").read_bytes() == (
        delta.numpy().astype("<f4", copy=False).tobytes()
    )
