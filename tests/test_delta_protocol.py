...
class RawLearner:
    ...
    def acknowledge_finalization(self) -> None:
        """Receive the exact terminal cut and ACK its versioned manifest."""
        manifest = None
        ...
        while time.monotonic() < deadline:
            ...
            if msg_type == MSG_ERROR:
                raise AssertionError(payload.decode("utf-8", errors="replace"))
            if msg_type == MSG_FINAL_FRAGMENT:
                fid, version = struct.unpack_from("<IQ", payload)
                fragments[fid] = version
            ...
            if manifest is not None and all(
                ...
                write_frame(
                    ...
                    MSG_FINAL_ACK,
                    struct.pack(
                        "<HQ", FINALIZATION_REVISION, manifest.global_step
                    ),
                )
                return
        raise TimeoutError("no complete final cut within 10s")
...
def test_fresh_and_one_version_stale_deltas_ignore_global_drift(
    ...
    try:
        ...
        learner.acknowledge_finalization()
...
def test_mid_round_join_cannot_count_toward_frozen_membership(
    ...
    try:
        ...
        first.acknowledge_finalization()
        joined.acknowledge_finalization()
        assert proc.wait(timeout=10) == 0
...
def test_out_of_range_learner_ids_are_rejected_at_startup_and_after_init(
    ...
    try:
        ...
        valid.acknowledge_finalization()
        assert proc.wait(timeout=10) == 0
        assert read_final_scalar(final_state) == pytest.approx(-1.0)
    finally:
