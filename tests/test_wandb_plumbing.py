...
def test_the_tape_path_is_shared_between_writer_and_reader():
    ...
    assert syncer.event_tape == os.path.expanduser(SYNCER_EVENT_TAPE)
...
def test_rl_and_sft_syncers_write_different_tapes_and_everyone_agrees():
    """RL puts the tape under the output dir it collects; SFT does not.

    syncer_command, LocalSyncer's strict-failure reader and the W&B
    forwarder must all resolve the same path, or the syncer run silently
    tails a file nothing is writing.
    """
    import os

    sft = _syncer_args(wandb=True)
    rl = _syncer_args(wandb=True, training_mode="rl", rl_total_fragment_steps=12)

    assert syncer_event_tape(sft) == SYNCER_EVENT_TAPE
    assert syncer_event_tape(rl) == RL_SYNCER_EVENT_TAPE
    assert SYNCER_EVENT_TAPE != RL_SYNCER_EVENT_TAPE

    for args, tape in ((sft, SYNCER_EVENT_TAPE), (rl, RL_SYNCER_EVENT_TAPE)):
        assert f"--event-tape {tape}" in syncer_command(args, 2)
        assert LocalSyncer(args, 2).event_tape == os.path.expanduser(tape)
        _setup, run, _envs = syncer_tape_sidecar(args, 2)
        assert f"yeto.wandb_tape {tape} --follow" in run
