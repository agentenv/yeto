...
def test_variant_plan_has_isolated_loss_and_attention_comparisons():
    assert [variant.name for variant in benchmark.VARIANTS] == [
        ...
        "native-sdpa-cudnn",
        "native-flash-attn-2",
        "fused-linear-ce-sdpa-math",
        "fused-linear-ce-sdpa-cudnn",
    ]
    ...
    selected = benchmark.select_variants(
        "fused-linear-ce-sdpa-math",
        reference_name="native-sdpa-math",
    )
    ...
    assert fused_loss.loss_implementation == benchmark.FUSED_LINEAR_CE_IMPLEMENTATION
    assert fused_loss.internal_sdpa_backend == "math"

    with pytest.raises(ValueError, match="loss-implementation comparisons"):
        benchmark.select_variants("fused-linear-ce-sdpa-math")
    with pytest.raises(ValueError, match="loss-implementation comparisons"):
        benchmark.select_variants(
            "fused-linear-ce-sdpa-cudnn",
            reference_name="native-sdpa-math",
        )
...
def test_fused_loss_variant_binds_instance_forward_before_peft(monkeypatch):
    ...
    loaded, tuning, kernel_application = benchmark.load_raw_model(
        ...
        "a" * 40,
        benchmark.VARIANTS_BY_NAME["fused-linear-ce-sdpa-math"],
        torch.bfloat16,
        torch.device("cpu"),
        "lora",
        16,
...
def test_every_kernel_isolation_failure_is_a_fatal_benchmark_load_error():
    ...
    assert not benchmark.is_fatal_model_load_error(
        RuntimeError("missing optional package")
    )
...
def test_failed_model_load_unconditionally_collects_instance_forward_cycle(
    ...
    monkeypatch.setattr(
        benchmark,
        "setup_distributed",
        lambda _timeout_seconds: (0, 1, torch.device("cpu")),
    )
    monkeypatch.setattr(
        benchmark,
    ...
    fused_scope = report["supported_evidence_scope"]["fused-linear-ce"]
    assert fused_scope["variants"] == [
        "fused-linear-ce-sdpa-math",
        "fused-linear-ce-sdpa-cudnn",
    ]
    assert fused_scope["peft_version"] == benchmark.PEFT_VERSION
    assert fused_scope["forward_source_sha256"] == benchmark.LIGER_QWEN2_SOURCE_SHA256
...
def test_benchmark_argument_validation_is_cpu_safe():
    ...
    broken = SimpleNamespace(**vars(args))
    broken.tuning = "full"
    broken.reference_variant = "native-sdpa-math"
    broken.variants = "fused-linear-ce-sdpa-math"
