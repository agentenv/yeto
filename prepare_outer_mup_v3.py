#!/usr/bin/env python3
"""Build expanded, seed-shuffled outer-muP v3 input bundles.

The frozen Day-1 development and audit rows remain byte-identical.  Training
uses every other source row, so a long-horizon run does not wrap its packed
dataset before 20,480 one-block learner steps.
"""
...
DEFAULT_SEEDS = tuple(range(301, 332)) + tuple(range(401, 410))
...
def main() -> int:
    ...
    parser.add_argument(
        "--out", type=Path, default=Path("/root/yeto-data/outer-mup-v3")
    )
    parser.add_argument(
    ...
    if any(seed not in DEFAULT_SEEDS for seed in seeds):
        raise SystemExit("seed is outside the requested 301..331 or 401..409 families")
    ...
    if set(train_pool) & (set(eval_indices) | set(audit_indices)):
        raise SystemExit("expanded training pool overlaps frozen evaluation rows")
    ...
    for seed in seeds:
        ...
        temporary = Path(tempfile.mkdtemp(prefix=f".seed-{seed}-", dir=out))
        try:
            ...
            provenance = {
                "schema": "yeto_split_provenance_v3",
            ...
            record = {
                "schema": "yeto_outer_mup_seed_input_v3",
                "seed": seed,
                "training_seed": int(f"{seed}{seed}"),
                "split_rule": {
                    "source_rows": SOURCE_ROWS,
                    "legacy_split_population_rows": LEGACY_POPULATION_ROWS,
                    "eval_split_seed": EVAL_SPLIT_SEED,
    ...
    write_json_atomic(out / "input-manifest.json", combined)
    print(
        json.dumps(
            {
