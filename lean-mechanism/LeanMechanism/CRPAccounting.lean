...
/-- Signed CRP residual under the convention used in this accounting module. -/
def crpResidual (G Q : E) : E :=
  G - Q

/-- The residual is exactly stock minus proposal. -/
theorem crpResidual_identity (G Q : E) : crpResidual G Q = G - Q :=
  rfl

/-- Subtracting the full residual from stock recovers the proposal. -/
theorem stock_sub_crpResidual (G Q : E) : G - crpResidual G Q = Q := by
  simp [crpResidual]
...
/-- The same expansion specialized to the signed residual `G - Q`. -/
theorem norm_sq_stock_sub_crpResidual (G Q : E) (lambda : ℝ) :
    ‖G - lambda • crpResidual G Q‖ ^ 2 =
      ‖G‖ ^ 2 - 2 * lambda * ⟪G, crpResidual G Q⟫_ℝ +
        lambda ^ 2 * ‖crpResidual G Q‖ ^ 2 :=
  norm_sq_stock_sub_residual G (crpResidual G Q) lambda
...
proposal interaction; it is stock energy minus the stock/proposal cross term. -/
theorem coordinateCross_crpResidual {n : ℕ} (G Q : Fin n → ℝ) :
    coordinateCross G (crpResidual G Q) =
      coordinateEnergy G - coordinateCross G Q := by
  unfold coordinateCross coordinateEnergy crpResidual
  calc
    ∑ i, G i * (G - Q) i = ∑ i, ((G i) ^ 2 - G i * Q i) := by
      apply Finset.sum_congr rfl
      intro i _
      simp only [Pi.sub_apply]
      ring
    _ = (∑ i, (G i) ^ 2) - ∑ i, G i * Q i := by
