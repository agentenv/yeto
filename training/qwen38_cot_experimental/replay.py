...
VERSION = "original-replay-to-native-xhigh-experimental/v1"
_renderer = None
...
def normalize_job(job):
    ...
    if job["format"] == "rollout":
        messages, counts, group = baseline.rollout_messages(
            [json.loads(line) for line in raw.splitlines() if line.strip()])
    else:
        messages, counts, group = baseline.atif_messages(json.loads(raw))
...
def convert_job(job, *, renderer=None):
    ...
    try:
        ...
        provenance = {**job, "replay_adapter_version": VERSION,
            ...
            "normalization_version": baseline.VERSION,
            "normalizer_sha256": baseline.sha_file(baseline.__file__),
