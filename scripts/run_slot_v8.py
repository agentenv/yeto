#!/usr/bin/env python3
"""Preflight or drain one hash-bound v8 phase-diagram GPU queue."""
...
def storage_proof() -> tuple[dict, list[str]]:
    ...
    try:
        ...
        if not RESULT_LINK.is_symlink():
            errors.append("v8 results path is not a symlink")
        if resolved != RESULT_TARGET:
            errors.append(f"v8 results resolve to {resolved}, not {RESULT_TARGET}")
        if not proof["same_device_as_data"]:
            errors.append("v8 results are not on the /data filesystem")
        if usage.free < MIN_FREE_BYTES:
            errors.append(f"v8 results filesystem has only {usage.free} bytes free")
...
def verify_inputs(manifest: dict, repo: Path, errors: list[str]) -> dict:
    ...
    verify_file_record(input_manifest_path, input_manifest_record, errors, "v8 input manifest")
    combined = {}
    if input_manifest_path.is_file():
        try:
            ...
            if combined.get("schema") != "yeto_outer_mup_v8_inputs_v1":
                errors.append("v8 input manifest schema mismatch")
            if sorted(map(int, combined.get("seeds", {}))) != [801, 809, 811, 821, 823]:
                errors.append("v8 input manifest seed set mismatch")
            ...
            errors.append(f"cannot validate v8 input manifest: {exc}")
    ...
    verify_file_record(
        ...
        errors,
        "v8 token-capacity report",
    )
    if capacity_path.is_file():
        try:
            report = json.loads(capacity_path.read_text())
            if report.get("status") != "PASS":
                errors.append("v8 token-capacity report is not PASS")
            minimum = report.get("minimum_across_all_seeds_and_learners", {}).get(
                "blocks"
            )
            required = report.get("pipeline", {}).get("required_blocks_per_learner")
            if not isinstance(minimum, int) or minimum < 20_480:
                errors.append(f"v8 token capacity is only {minimum}")
            if required != 20_480:
                errors.append(f"v8 capacity report required-block count is {required}")
        except Exception as exc:
            errors.append(f"cannot validate v8 capacity report: {exc}")
...
def verify_preflight(manifest_path: Path, node_label: str, proof_path: Path) -> dict:
    ...
    try:
        ...
        if manifest["registration"]["git_commit"] != head:
            errors.append("v8 must execute at its registration commit")
    except Exception as exc:
        errors.append(f"Git preflight failed: {exc}")

    contract = manifest.get("contract", {})
    contract_paths = {
        "json_sha256": repo / "experiment-specs/outer-mup-v8-phasediagram-prereg.json",
        "md_sha256": repo / "experiment-specs/outer-mup-v8-phasediagram-prereg.md",
        "analyzer_sha256": repo / "scripts/analyze_v8.py",
    }
    for field, path in contract_paths.items():
        if not path.is_file() or sha256_file(path) != contract.get(field):
    ...
    for cell in cells:
        ...
        if cell.get("h") != 512 or cell.get("s") != 512 * cell.get("t", 0):
        ...
        for flag in ("--work-dir", "--report-dir"):
            ...
            if value is None or not value.startswith(str(RESULT_LINK) + "/"):
                errors.append(f"{flag} escapes roomy v8 results: {cell_id}")
    ...
    proof = {
        "schema": "yeto_outer_mup_v8_node_authority_v1",
        "node": node_label,
...
def load_gate_proof(path: Path) -> dict:
    ...
    for node in ("h200-n1", "h200-n2"):
        ...
        if node_record.get("result_root_absent") is not True and not node_record.get(
            ...
            errors.append(f"gate proof lacks a v6 drain condition on {node}")
    if errors:
        raise SystemExit("; ".join(errors))
    return proof
...
def load_retry_authority(path: Path, manifest_path: Path, manifest: dict) -> set[str]:
    authority = json.loads(path.read_text())
    errors = []
    if authority.get("schema") != "yeto_outer_mup_v8_retry_authority_v1":
    ...
    if authority.get("reason") not in set(
        manifest.get("retry_contract", {}).get("allowed_retry_reasons", [])
    ):
...
def run_queue(
    ...
    status_path = (
        RESULT_LINK / "_controller" / "slots-v8" / f"{node_label}-gpu{gpu}.json"
    )
    lock_path = (
        RESULT_LINK / "_controller" / "locks-v8" / f"{node_label}-gpu{gpu}.lock"
    )
