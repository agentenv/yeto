#!/usr/bin/env python3
"""Verify v6 shard integrity and exact SmolLM2 packed-token capacity."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--root", type=Path, default=Path("/root/yeto-data/outer-mup-v6")
    )
    parser.add_argument(
    ...
    parser.add_argument("--steps", type=int, default=10_240)
    ...
    if combined.get("schema") != "yeto_outer_mup_v6_inputs_v1":
        raise SystemExit("v6 input manifest schema mismatch")
    observed_seeds = tuple(sorted(int(seed) for seed in combined["seeds"]))
    if observed_seeds != V6_SEEDS:
    ...
    result = {
        "schema": "yeto_outer_mup_v6_token_verification_v1",
        "status": "PASS",
        "pipeline": {
