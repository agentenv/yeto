...
def select_variants(
    ...
    for candidate in selected_candidates:
        if candidate.loss_implementation == reference.loss_implementation:
            continue
        isolation_fields = (
            "attention_backend",
            "internal_sdpa_backend",
            "layer_backend",
        )
        mismatches = {
        ...
        if mismatches:
            raise ValueError(
                "loss-implementation comparisons require identical attention, "
                "internal SDPA, and layer backends; "
                f"reference={reference.name!r} candidate={candidate.name!r} "
                f"mismatches={mismatches}"
            )
    return [reference, *selected_candidates]
...
def load_raw_model(
    ...
    production_envelope = (
        validate_lora_production_envelope(model) if tuning == "lora" else None
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "optional single candidate name; the selected reference is added "
            "automatically"
        ),
    )
    parser.add_argument(
...
def validate_args(args) -> None:
    ...
    selected = select_variants(args.variants, args.reference_variant)
