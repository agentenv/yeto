#!/usr/bin/env python3
"""Build the frozen, no-wrap training bundles for outer-muP v6.

The source partition and fixed development/audit sets are byte-identical to
v3.  Only the prospectively registered v6 training shuffle seeds differ.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--out", type=Path, default=Path("/root/yeto-data/outer-mup-v6")
    )
    ...
    temporary_root = Path(tempfile.mkdtemp(prefix=".outer-mup-v6-", dir=out.parent))
    try:
        ...
        for seed in V6_SEEDS:
            ...
            provenance = {
                "schema": "yeto_split_provenance_v6",
            ...
            record = {
                "schema": "yeto_outer_mup_seed_input_v6",
                "seed": seed,
        ...
        combined = {
            "schema": "yeto_outer_mup_v6_inputs_v1",
            "source": {
                "path": str(source),
