#!/usr/bin/env python3
"""Authorize one loss-blind whole-curve v5 infrastructure retry wave."""
...
def main() -> int:
    ...
    if (
        ...
        raise SystemExit("not the complete v5 launch manifest")
    ...
    if set(group_sizes.values()) != {6}:
        raise SystemExit(f"retry groups are not complete six-eta curves: {group_sizes}")
    authority = {
        "schema": "yeto_outer_mup_v5_retry_authority_v1",
        "status": "AUTHORIZED",
