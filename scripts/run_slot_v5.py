#!/usr/bin/env python3
"""Preflight or drain one hash-bound v5 SNOO GPU queue."""
...
CONTRACT_JSON_SHA256 = "ed5b9fc2460c7d076900a3f790cf4efff734349c42c5b68f057bd4079d591b43"
CONTRACT_MD_SHA256 = "aed5d4a838a7f5b2c38e47c01bae50900ebb19bcd041dbc6d47f88f69b9aeeac"
ANALYZER_SHA256 = "e574294d1d31ad9f8e7431168a877bd4fb58ac5c7c628d7e882eb8a89414a97e"
RESULT_LINK = Path("/root/yeto-results-v5")
RESULT_TARGET = Path("/data/yeto-results-v5")
...
def v4_process_ids() -> list[int]:
    result = subprocess.run(
        ["pgrep", "-f", "run_slot_v[4]"], capture_output=True, text=True
    )
    if result.returncode == 1:
        return []
    if result.returncode != 0:
        raise RuntimeError(result.stderr.strip() or "pgrep run_slot_v4 failed")
...
def storage_proof() -> tuple[dict, list[str]]:
    ...
    try:
        ...
        if not RESULT_LINK.is_symlink():
            errors.append("v5 results path is not a symlink")
        if resolved != RESULT_TARGET:
            errors.append(f"v5 results resolve to {resolved}, not {RESULT_TARGET}")
        if not proof["same_device_as_data"]:
            errors.append("v5 results are not on the /data filesystem")
        if usage.free < MIN_FREE_BYTES:
            errors.append(f"v5 results filesystem has only {usage.free} bytes free")
...
def verify_inputs(inputs: dict) -> list[str]:
    ...
    if not seed_path.is_file():
        return [f"missing v5 seed manifest: {seed_path}"]
    if sha256_file(seed_path) != seed_binding.get("sha256"):
        return ["v5 seed manifest hash mismatch"]
    try:
        observed = json.loads(seed_path.read_text())
    except Exception as exc:
        return [f"cannot parse v5 seed manifest: {exc}"]
    if observed != seed_binding.get("contents"):
        errors.append("v5 seed manifest differs from hash-bound manifest contents")
    if sorted(int(seed) for seed in observed.get("seeds", {})) != list(SEEDS):
        errors.append("v5 seed manifest has the wrong seed set")
    ...
    if not development_path.is_file() or sha256_file(development_path) != development.get("sha256"):
...
def verify_preflight(manifest_path: Path, node_label: str, proof_path: Path) -> dict:
    ...
    if manifest.get("manifest_variant") != "v5_snoo_interior":
        errors.append("launch manifest variant mismatch")
    if manifest.get("stage") != "V5_SNOO_INTERIOR" or len(manifest.get("cells", [])) != 90:
        errors.append("launch manifest is not the complete 90-cell v5 stage")
    ...
    try:
        ...
        if manifest["registration"]["git_commit"] != head:
            errors.append("v5 must execute at its registration commit")
    except Exception as exc:
    ...
    contract_paths = {
        "json_sha256": repo / "experiment-specs/outer-mup-v5-snoo-interior-prereg.json",
        "md_sha256": repo / "experiment-specs/outer-mup-v5-snoo-interior-prereg.md",
        "analyzer_sha256": repo / "scripts/analyze_v5.py",
    }
    ...
    for cell in manifest.get("cells", []):
        ...
        elif abs(float(cell.get("eta")) - ETA_GRIDS[condition][eta_index]) > 1e-15:
        ...
        if "--rho-telemetry" not in command or "--outer-bias-correction" in command:
            errors.append(f"v5 telemetry/bias-correction contract mismatch: {cell_id}")
        ...
        for flag in ("--work-dir", "--report-dir"):
            ...
            if value is None or not value.startswith(str(RESULT_LINK) + "/"):
                errors.append(f"{flag} escapes v5 results: {cell_id}")
    for eta_b, eta_c in zip(ETA_GRIDS["b"], ETA_GRIDS["c"]):
        if abs(eta_c - eta_b * CODE_TRUE_MATCH_FACTOR) > 1e-15:
            errors.append("code-true condition-c eta pairing mismatch")
    ...
    try:
        active_v4 = v4_process_ids()
        if active_v4:
            errors.append(f"v4 slot controllers still active: {active_v4}")
    except Exception as exc:
        active_v4 = []
        errors.append(f"v4 process gate failed: {exc}")
    storage, storage_errors = storage_proof()
    errors.extend(storage_errors)
    proof = {
        "schema": "yeto_outer_mup_v5_node_authority_v1",
        "node": node_label,
...
def load_launch_authority(path: Path, manifest_path: Path) -> dict:
    ...
    errors = []
    if authority.get("schema") != "yeto_outer_mup_v5_launch_authority_v1":
    ...
    if authority.get("v4_drain_evidence", {}).get("status") != "PASS":
        errors.append("launch authority lacks a passing v4 drain proof")
...
def load_retry_authority(path: Path, manifest_path: Path, manifest: dict) -> set[str]:
    authority = json.loads(path.read_text())
    errors = []
    if authority.get("schema") != "yeto_outer_mup_v5_retry_authority_v1":
...
def run_queue(
    ...
    if v4_process_ids():
        raise SystemExit("v4 slot controller appeared after v5 preflight")
    ...
    status_path = RESULT_LINK / "_controller" / "slots-v5" / f"{node_label}-gpu{gpu}.json"
    lock_path = RESULT_LINK / "_controller" / "locks-v5" / f"{node_label}-gpu{gpu}.lock"
    ...
    with lock_path.open("w") as lock:
        ...
        for queue_index, cell in enumerate(queue):
            ...
            write_json_atomic(
                status_path,
                {
                    "schema": "yeto_outer_mup_v5_slot_status_v1",
                    "state": "RUNNING",
                    "node": node_label,
                    "gpu": gpu,
                    "cell_id": cell["cell_id"],
