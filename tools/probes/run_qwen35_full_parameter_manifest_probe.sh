...
# Derives the real TP2 owner-fragment plan. It starts no SGLang or train loop.
set -euo pipefail

: "${YETO_FULL_PARAMETER_MANIFEST_PROBE_EVIDENCE:?missing manifest-probe evidence path}"
...
: "${YETO_FULL_PARAMETER_YETO_SOURCE_SHA256:?missing Yeto source hash}"
: "${YETO_FULL_PARAMETER_MILES_SOURCE_SHA256:?missing Miles source hash}"
...
test ! -e "${YETO_FULL_PARAMETER_MANIFEST_PROBE_EVIDENCE}"
...
export YETO_FULL_PARAMETER_CONVERSION_MANIFEST_PATH=/models/torch_dist/conversion-manifest.json
export YETO_FULL_PARAMETER_YETO_SOURCE_ROOT=/root/yeto
export YETO_FULL_PARAMETER_MILES_SOURCE_ROOT=/root/miles

ray_temp=/evidence/ray-manifest-probe
test ! -e "${ray_temp}"
...
names = (
    ...
    "YETO_FULL_PARAMETER_CONVERSION_MANIFEST_PATH",
    "YETO_MILES_IMAGE_DIGEST",
    "YETO_FULL_PARAMETER_YETO_SOURCE_ROOT",
    "YETO_FULL_PARAMETER_YETO_SOURCE_SHA256",
    "YETO_FULL_PARAMETER_MILES_SOURCE_ROOT",
    "YETO_FULL_PARAMETER_MILES_SOURCE_SHA256",
)
...
# The fragment plan must be derived from the exact production model
# construction profile. Qwen3.5 has sequence-length-sensitive modules, so a
# reduced metadata-probe length is not a faithful layout attestation.
ray job submit \
  --address=http://127.0.0.1:8267 \
