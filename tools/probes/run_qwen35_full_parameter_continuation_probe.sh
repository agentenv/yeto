...
# Runs inside a fresh dedicated Miles container. It starts no SGLang process.
set -euo pipefail

: "${YETO_FULL_PARAMETER_CONTINUATION_EVIDENCE:?missing continuation evidence path}"
...
test -f /root/yeto/tests/fixtures/qwen35_full_parameter_sft_probe.jsonl
test ! -e "${YETO_FULL_PARAMETER_CONTINUATION_EVIDENCE}"
...
ray_temp=/evidence/ray-continuation
test ! -e "${ray_temp}"
...
names = (
    ...
    "YETO_FULL_PARAMETER_CONTINUATION_EVIDENCE",
...
ray job submit \
  ...
  --prompt-data /root/yeto/tests/fixtures/qwen35_full_parameter_sft_probe.jsonl \
  --input-key messages \
