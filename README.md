...
- `nava` expands to `--task diffusion --component nava`; default config `configs/nava.yaml`; default adapter `lora`; default LoRA targets `mmdit-all-linear`; runtime paths/images may be supplied by `YETO_NAVA_BASE_CHECKPOINT`, `YETO_NAVA_ROOT`, `YETO_NAVA_LEARNER_IMAGE`.
