...
**任务 2.1 的核查结论（2026-09-24）**：转换路径不接受"传一个张量"，但也不要求
原来的那个 `Parameter` 对象——它要求的是**模块属性**。

- 导出侧：`bridge.export_adapter_weights` →
  `stream_adapter_weights_megatron_to_hf` 每次都重新调用
  `build_adapter_conversion_tasks(model)`，后者用
  `adapter.linear_in.weight` / `adapter.linear_out.weight` 属性查找取张量，
  再把它交给 `mapping.megatron_to_hf(task.param_weight, module)`。所以
  `_collective_adapter_tensors` 里预先算好的 `side.param_weight` 并不会被
  bridge 读到，别名到 `Parameter.data` 只是为了让这次属性查找看到 fp32。
- 应用侧：`hf_to_megatron(value, module)` 在 `tp_size > 1` 时用
  `get_module_and_param_from_name(module, ...)` 取目标参数，并把输入 cast 到
  它的 dtype 再 scatter；`tp_size == 1` 时原样返回。没有 fp32 视图，TP>1 会把
  fp32 canonical 值先截成 bf16 再写进 fp32 master，改变数值。
- `patch_megatron_model` 只动 config 属性和 expert bias，与参数对象无关。

因此落地写法是 D1 的精神、但机制略有不同：context manager 在每个 side 的
`megatron_module._parameters[<attr>]` 上临时放一个**新建的**
`torch.nn.Parameter`（共享 `main_param.view(shape)` 的存储，`requires_grad=False`），
退出时把原 `Parameter` 对象放回去。原 `Parameter` 的存储、dtype、autograd
meta 全程不被触碰，梯度累加器身份不变；bridge 的属性查找和 dtype cast 看到
的都是 fp32。两条路径喂给 `megatron_to_hf` / `copy_` 的都是同一块
`main_param` 存储，导出数值与修复前逐位相同（Miles 单测断言
`exposed.data_ptr() == main_param.data_ptr()`）。不需要 D1 的兜底方案。

**任务 2.4 的结论**：不能直接删掉这个 context manager。写入侧确实直接操作
`main_param`，但读取/转换侧仍有两处依赖它：导出时 bridge 的属性查找必须看到
fp32 master（否则导出的是 bf16 模型副本，canonical state 会丢精度）；应用时
TP>1 的 `hf_to_megatron` 会按模块参数的 dtype cast。保留，但改成上述不触碰
`Parameter` 的实现。
